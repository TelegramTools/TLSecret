...
def EventHandler(event):
    if "WooHoo!" in event.raw_text:
        print("\nResponse received!")
        client1.delete_messages(ChosenChat, [file.id, event.original_message.id], revoke=True)
        client1.disconnect()
    return
...
if not client1.is_user_authorized():
    ...
    client1.disconnect()
    pyAesCrypt.encryptFile("User2.session", "DB.aes", password, bufferSize)
    StartClient1()
